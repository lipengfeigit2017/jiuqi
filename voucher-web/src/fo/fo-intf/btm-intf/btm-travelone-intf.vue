...
export default {
    ...
    methods: {
        async handleGetInftInfo () {
            const {data} = await this.$http.post('/api/cfasserver/btm/intf/get?agent=TravelOne')
        ...
        async handleInit () {
            try {
                const {data} = await this.$http.post('/api/cfasserver/btm/intf/init?agent=TravelOne')
                if (data.code === 0) {
        ...
        async handleSync () {
            try {
                const {data} = await this.$http.post('/api/cfasserver/btm/intf/sync?agent=TravelOne')
                if (data.code === 0) {

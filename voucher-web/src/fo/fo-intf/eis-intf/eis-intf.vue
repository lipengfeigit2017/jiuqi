...
export default {
    ...
    methods: {
        async handleGetInftInfo () {
            const {data} = await this.$http.post('/api/cfasserver/eis/intf/get')
        ...
        async handleSave () {
            try {
                ...
                const {data} = await this.$http.post('/api/cfasserver/eis/intf/patch', this.originalData)
                if (data.code === 0) {
                    this.isEdit = false
                    this.handleGetInftInfo()
        ...
        async handleInit () {
            try {
                const {data} = await this.$http.post('/api/cfasserver/eis/intf/init')
                if (data.code === 0) {
        ...
        async handleSync () {
            try {
                const {data} = await this.$http.post('/api/cfasserver/eis/intf/sync')
                if (data.code === 0) {
